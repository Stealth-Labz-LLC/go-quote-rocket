...
function motorApiCall($data, $base_url, $updateTime, $preferredtimeofcall)
{
    ...
    $manufacturerWarranty = !empty($data['manufacturer_warranty']) ? $data['manufacturer_warranty'] : 'false';
    $vehicleMileage = !empty($data['milage']) ? $data['milage'] : '';
    $vehicleAge = !empty($data['vehicle-age']) ? $data['vehicle-age'] : '';

    $vehicleAgeBar = ['0-5', '6-10', '11-15'];
    if ($income == 'R0 - R7999' || $manufacturerWarranty != 'true' || $vehicleMileage == '250 001+ KM' || !in_array($vehicleAge, $vehicleAgeBar)) {
        $errorResponse = [
            "status" => "Validation Error",
            "code" => 2,
