...
class FunnelEngine {
    ...
    renderContactForm(question) {
        ...
        const newsletterHtml = question.newsletter && question.newsletter.enabled ? `
            <div class="newsletter-checkbox">
                ...
                >
                <label for="newsletter">${question.newsletter.text}</label>
            </div>
        ` : '';
    ...
    updateNextButton() {
        ...
        if (question.type === 'text') {
            const input = this.container.querySelector(`#input_${question.id}`);
            isValid = this.validateField(question, input.value);
            this.userData[question.id] = input.value;
        }
    ...
    updateNavigation() {
        ...
        if (question.type === 'contact_form' && question.submit_text) {
            this.nextBtn.textContent = question.submit_text;
        } else {
    ...
    showLoadingModal() {
        ...
        const interval = setInterval(() => {
            if (messageIndex < this.loadingMessages.length) {
                messageEl.textContent = this.loadingMessages[messageIndex];
                messageIndex++;
            }
        }, 800);
